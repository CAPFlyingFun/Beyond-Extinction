...
export const SETTINGS_RANGES = {
  fov: { min: 40, max: 75, step: 1, default: 55 },
  zoom: { min: 0.7, max: 1.8, step: 0.01, default: 1 },
} as const;
...
// v2: the camera control switched from a "distance" multiplier (higher = far)
// to an intuitive "zoom" multiplier (higher = close), so old values must not be
// reused — bumping the key resets stale prefs to the new defaults.
const STORAGE_KEY = "beyond-extinction.settings.v2";
